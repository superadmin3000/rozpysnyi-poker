...
function render(){
 ...
 $('phase').textContent=phaseText(state.phase);
 (function renderTrump(){
   const tc=state.trumpCard;
   if(tc && !tc.joker && state.trump){
     const isRed=tc.suit==='♥'||tc.suit==='♦';
     $('trump').innerHTML=`<div class="trump-label">Козир</div><div class="card trump-face ${isRed?'red':''}" title="Козир: ${esc(tc.rank)}${tc.suit}"><span class="trump-rank">${esc(tc.rank)}</span><span class="trump-suit-big">${tc.suit}</span></div>`;
   } else if(tc && tc.joker){
     $('trump').innerHTML=`<div class="trump-label">Козир</div><div class="card trump-face joker" title="Джокер — без козиря"><span class="trump-rank">🃏</span><span class="trump-suit-big">—</span></div>`;
   } else if(state.trump){
     const isRed=state.trump==='♥'||state.trump==='♦';
     $('trump').innerHTML=`<div class="trump-label">Козир</div><div class="card trump-face ${isRed?'red':''}"><span class="trump-suit-big">${state.trump}</span></div>`;
   } else {
     $('trump').innerHTML=`<div class="trump-label muted">Без козиря</div>`;
   }
 })();
 ...
 if(state.phase==='lobby')info=`Очікуємо 4 гравців (${state.players.length}/4)`;
   ...
   const top=sorted[0];
   info=top?`Партію завершено · Переможець: ${top.name} (${top.score})`:'Партію завершено';
 }
 $('info').textContent=info;
 renderActions(me,myTurn);renderHand();renderScore();$('log').innerHTML=state.log.map(esc).join('<br>');
}
...
function playerHTML(p,i){
  ...
  return `<div class="player-card ${isTurn?'active':''} ${isDealer?'is-dealer':''}">
    ...
    <div class="player-meta">${p.bot?'🤖 бот':(p.connected?'● онлайн':'⚠ відключився')} · ${p.cards} карт · ${p.tricks} вз.</div>
    ${p.bid!==null?`<div class="player-meta">Замовлення: <b>${p.bid}</b></div>`:''}
...
function renderActions(me,myTurn){
 ...
 if(state.phase==='roundEnd'&&isHost) a+='<button onclick="nextRound()">Наступний раунд</button>';
 // Кнопки господаря кімнати під час/після гри
 if(isHost && state.phase!=='lobby'){
   a+='<button class="primary-btn" onclick="newGame()">Почати гру по новій</button> ';
   a+='<button class="secondary danger-btn" onclick="endGame()">Завершити гру</button> ';
 }
 if(state.phase==='gameEnd'){
   a+='<button class="secondary" onclick="leaveGame()">Вийти з гри</button>';
 }
 if(state.publicUrl){
   a+=`<div class="public-url">🌐 <a href="${esc(state.publicUrl)}" target="_blank" rel="noopener">${esc(state.publicUrl)}</a> <button class="secondary" type="button" onclick="copyPublicUrl()" style="padding:4px 8px;font-size:12px">Копіювати</button></div>`;
 }
 $('actions').innerHTML=a;
}
function shareLink(){
...
function renderHand(dealAnimate){
 ...
 $('hand').innerHTML=hand.map((c,i)=>
   `<div class="card ${red(c)?'red':''} ${c.joker?'joker':''} ${!legal.has(c.id)?'disabled':''} ${dealAnimate?'deal-in':''}"
        style="${dealAnimate?`--deal-i:${i}`:''}"
        onclick='play(${JSON.stringify(c.id)})'>${cardText(c)}</div>`
 ).join('');
 $('handHint').textContent=state.phase==='playing'&&myTurn?'Ваш хід':'';
 if(dealAnimate){
   // Легкий «розліт» з центру столу під час роздачі
   animateDealBurst(hand.length);
   setTimeout(()=>{
     if(token!==dealAnimToken) return;
     document.querySelectorAll('#hand .deal-in').forEach(n=>n.classList.remove('deal-in'));
   }, 900);
 }
}
...
function appendChat(entry,system){
  if(!chatEl) return;
  const div=document.createElement('div');
  div.className='chat-msg'+(system?' system':'');
  if(system){ div.textContent=entry; }
  else {
    const t=new Date(entry.ts||Date.now());
    const time=t.toLocaleTimeString('uk-UA',{hour:'2-digit',minute:'2-digit'});
    div.innerHTML=`<span class="who">${esc(entry.name)}</span>${esc(entry.text)}<span class="when">${time}</span>`;
  }
  chatEl.appendChild(div);
  chatEl.scrollTop=chatEl.scrollHeight;
}
socket.on('chat',entry=>appendChat(entry));
// Load history when state arrives first time
let chatLoaded=false;
const _origRender=render;
render=function(){
  _origRender();
  if(state&&state.chat&&!chatLoaded){
    chatLoaded=true;
    chatEl.innerHTML='';
    state.chat.forEach(m=>appendChat(m));
  }
  updateVoiceUI();
};
$('chatForm').onsubmit=e=>{
